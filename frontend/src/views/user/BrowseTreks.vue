<template>
  <div class="app-layout">
    <SidebarNav />
    <div class="main-content split-main-layout">
      <!-- Animated mountain mist background -->
      <canvas id="mist-canvas" class="mist-bg"></canvas>
      <div class="topbar">
        <div>
          <div class="page-title" style="display:flex;align-items:center;gap:8px">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:24px;height:24px"><polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"></polygon><line x1="9" y1="3" x2="9" y2="18"></line><line x1="15" y1="6" x2="15" y2="21"></line></svg>
            Discover Trails
          </div>
          <div class="page-subtitle">Interactive route explorer for India pilgrimages and trails</div>
        </div>
      </div>

      <div class="split-container">
        <!-- 1. Left Panel: Routes Sidebar -->
              ...
              <div class="route-info">
                ...
                <div class="route-specs">
                  <span style="display:inline-flex;align-items:center;gap:4px">
                    <svg viewBox="0 0 24 24" fill="none" stroke="var(--jatayu-gold)" stroke-width="2.2" style="width:14px;height:14px;flex-shrink:0"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                    {{ t.duration_days }} Days
                  </span>
                  <span style="display:inline-flex;align-items:center;gap:4px">
                    <svg viewBox="0 0 24 24" fill="none" stroke="var(--jatayu-gold)" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;flex-shrink:0"><path d="M8 3l4 8 5-5 5 15H2L8 3z"></path></svg>
                    {{ t.max_altitude_m }}m
                  </span>
                  <span style="display:inline-flex;align-items:center;gap:4px">
                    <svg viewBox="0 0 24 24" fill="none" stroke="var(--jatayu-gold)" stroke-width="2.2" style="width:14px;height:14px;flex-shrink:0"><rect x="2" y="4" width="20" height="16" rx="2"></rect><line x1="12" y1="10" x2="12" y2="14"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg>
                    ₹{{ t.price_inr }}
                  </span>
                </div>
              </div>
        ...
        <transition name="slide-panel">
          <div v-if="selectedTrek" class="details-panel" :style="{
            backgroundImage: `linear-gradient(to bottom, rgba(8, 20, 12, 0.94), rgba(4, 10, 6, 0.98)), url(${selectedTrek.cover_image || fallbackImg})`,
            ...
            <div class="details-header">
              ...
              <div class="image-grid">
                <div class="grid-main">
                  <img :src="hoveredImage || (trekGalleryImages.length ? trekGalleryImages[0] : fallbackImg)" :alt="selectedTrek.name" class="main-preview-img" />
                </div>
                <div class="grid-side" v-if="trekGalleryImages.length > 2">
                  <img 
                    :src="trekGalleryImages[1]" 
                    alt="Trail peak" 
                    @mouseenter="hoveredImage = trekGalleryImages[1]"
                    @mouseleave="hoveredImage = null"
                    :class="{ active: hoveredImage === trekGalleryImages[1] }"
                  />
                  <img 
                    :src="trekGalleryImages[2]" 
                    alt="Mountain pass" 
                    @mouseenter="hoveredImage = trekGalleryImages[2]"
                    @mouseleave="hoveredImage = null"
                    :class="{ active: hoveredImage === trekGalleryImages[2] }"
                  />
                </div>
              </div>
            </div>
            ...
            <div class="details-body">
              ...
              <div class="specs-grid">
                <div class="spec-box">
                  <span class="icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="var(--jatayu-gold)" stroke-width="2" style="width:16px;height:16px"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                  </span>
                ...
                <div class="spec-box">
                  <span class="icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="var(--jatayu-gold)" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"></polyline><polyline points="17 6 23 6 23 12"></polyline></svg>
                  </span>
                ...
                <div class="spec-box">
                  <span class="icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="var(--jatayu-gold)" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"></rect><line x1="12" y1="10" x2="12" y2="14"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg>
                  </span>
              ...
              <div class="action-buttons-row">
                <button class="btn-action-outline" @click="downloadGPXFile(selectedTrek)">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:4px"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                  Download GPX
                </button>
...
const trekGalleryImages = computed(() => {
  if (!selectedTrek.value) return []
  const name = selectedTrek.value.name.toLowerCase()
  if (name.includes('brahmatal')) {
    return [
      selectedTrek.value.cover_image || '/brahmatal.png',
      '/brahmatal_lake.png',
      '/brahmatal_camp.png'
    ]
  }
  if (name.includes('chadar')) {
    return [
      selectedTrek.value.cover_image || '/chadar.jpg',
      '/chadar_frozen_river.png',
      '/chadar_campsite.png'
    ]
  }
  if (name.includes('dayara')) {
    return [
      selectedTrek.value.cover_image || '/dayara.png',
      '/gorson_bugyal.jpg',
      '/harela_festival.jpg'
    ]
  }
  if (name.includes('sandakphu')) {
    return [
      selectedTrek.value.cover_image || '/sandakphu.jpg',
      '/sandakphu_clouds.jpg',
      '/sandakphu_sunset.png'
    ]
  }
  if (name.includes('goechala')) {
    return [
      selectedTrek.value.cover_image || '/goechala.png',
      '/goechala_flags.png',
      '/goechala_lakes.png'
    ]
  }
  if (name.includes('kedarnath')) {
    return [
      selectedTrek.value.cover_image || '/kedarnath.jpg',
      '/chorabari_lake.png',
      '/bhairav_temple.png'
    ]
  }
  if (name.includes('kuari')) {
    return [
      selectedTrek.value.cover_image || '/kuari.png',
      '/kuari_oak_forest.png',
      '/gorson_bugyal.jpg'
    ]
  }
  if (name.includes('hemkund')) {
    return [
      selectedTrek.value.cover_image || '/hemkund.png',
      '/laxman_temple.png',
      '/hemkund_gurudwara.png'
    ]
  }
  if (name.includes('hampta')) {
    return [
      selectedTrek.value.cover_image || '/hampta.png',
      '/shea_goru.png',
      '/hampta_river_crossing.png'
    ]
  }
  return [
    selectedTrek.value.cover_image || fallbackImg,
    'https://images.unsplash.com/photo-1544829099-b9a0c07fad1a?w=300&q=80&fit=crop',
    'https://images.unsplash.com/photo-1454496522488-7a8e488e8606?w=300&q=80&fit=crop'
  ]
})
...
.split-main-layout {
  ...
  padding: 0 !important;
  position: relative;
  /* Relatable & High-Class Ambient Background: Dark Misty Mountain Peaks Backdrop with Gold & Green Glows */
  background: 
    radial-gradient(circle at 15% 15%, rgba(232, 168, 56, 0.08) 0%, transparent 45%),
    radial-gradient(circle at 85% 85%, rgba(80, 210, 140, 0.07) 0%, transparent 50%),
    linear-gradient(to bottom, rgba(5, 12, 8, 0.93) 0%, rgba(2, 6, 4, 0.98) 100%),
    url('https://images.unsplash.com/photo-1486873249359-2731bd6dafc7?w=1600&q=80') center/cover no-repeat fixed;
}
.mist-bg {
  position: absolute;
  inset: 0;
...
.topbar { 
  ...
  padding: 14px 24px; 
  border-bottom: 0.5px solid rgba(80, 210, 140, 0.08); 
  height: 65px;
  flex-shrink: 0;
  background: linear-gradient(90deg, rgba(4, 14, 8, 0.88) 0%, rgba(8, 22, 14, 0.82) 60%, rgba(4, 12, 7, 0.88) 100%);
  backdrop-filter: blur(20px) saturate(120%);
  position: relative;
  z-index: 2;
  box-shadow: 0 1px 0 rgba(80, 210, 140, 0.05), 0 4px 30px rgba(0,0,0,0.45);
}
.page-title {
  color: #fff;
  font-size: 18px;
  ...
  text-shadow: 0 0 20px rgba(80,210,140,0.2), 0 0 40px rgba(80,210,140,0.05);
...
.sidebar-panel {
  ...
  display: flex;
  flex-direction: column;
  border-right: 1px solid rgba(80, 210, 140, 0.06);
  background: linear-gradient(180deg,
    rgba(4, 12, 8, 0.85) 0%,
    rgba(6, 18, 12, 0.80) 30%,
    rgba(4, 12, 8, 0.85) 70%,
    rgba(2, 6, 4, 0.90) 100%
  );
  ...
  flex-shrink: 0;
  position: relative;
  box-shadow: inset -1px 0 0 rgba(80, 210, 140, 0.04), 5px 0 25px rgba(0, 0, 0, 0.3);
}
.search-filters {
  padding: 16px 20px;
  display: flex;
